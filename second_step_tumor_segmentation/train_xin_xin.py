...
def train_net(net,
    ...
    train_edg_dir = './train_Edge/'
    val_img_dir = './val_Image/'
    val_msk_dir = './val_Mask/'
    val_edg_dir = './val_Edge/'
...
Starting training:
    ...
    for epoch in range(epochs):
        ...
        sub_iter_num = 0
        kits = Kits19Dataset(imgPath=train_img_dir, mskPath=train_msk_dir, edgPath=train_edg_dir)
        ...
        for i, data in enumerate(train_loader):
            ...
            loss, boundary_loss = Joint_seg_edg_loss(masks_pred, true_masks, edges_pred, true_edges)##########
            print('iter:' + str(sub_iter_num) + ' ' + str('%4f' % loss.item()) + ' ' + str('%4f' % boundary_loss.item()))
            sub_iter_num += 1
            ...
            if (i+1) % subnums == 0 and i != 0:
                print('Epoch {} SubEpoch {} finished ! Loss: {} Boundary Loss: {}'.format(epoch, iter_num, epoch_loss / subnums, epoch_boundary_loss / subnums))
                epoch_loss_list.append(epoch_loss / subnums)
                info_list.append(epoch_loss / subnums)

                with torch.no_grad():
                    val_dice, val_loss = eval_net(net, val_img_dir, val_msk_dir, val_edg_dir, batch_size,  gpu)
                ...
                epoch_boundary_loss = 0##########
                sub_iter_num = 0
                info_txt.writelines('train-tum-loss:' + str(info_list[0]) + ' ' + 'val-tum-loss:' + str(info_list[1]) + ' ' + 'val-tum-dice-acc:' + str(info_list[2]) + '\n')
    ...
    plt.plot(x_new, y_1, color = 'green', label = 'train loss')
    plt.plot(x_new, y_2, color = 'red', label = 'val loss')
    plt.legend()
